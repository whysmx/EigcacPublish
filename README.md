...
2) 在 VS 中打开需要发布的解决方案  
3) 点击“Eigcac发布”命令（顶部菜单 `Eigcac发布`、Standard 工具栏按钮；同时会尝试注册在 `工具` 菜单、`项目` 菜单、解决方案右键菜单；快捷键 `Ctrl+Alt+Shift+P`）  
...
TFS 相关配置（可选，默认自动识别，只有权限不足时才需要填写）：
- 集合地址：指定 TFS Collection（为空会从当前工作区自动识别）  
- 服务器路径：例如 `$/CM-WinterFresh/源代码/Trunk/Eigcac`（为空会自动识别）  
- 用户名/密码：当 TFS 更新提示无权限时会弹窗要求填写，并保存，用于 `/login` 执行 `tf get`  
